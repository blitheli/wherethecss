...
import { AgXToneMapping, MathUtils, Vector3, type Object3D } from "three";
...
/*
  ...
  3D Tiles插件实现地球全球视角, 通过坐标系原点切换的方式实现飞机在世界系原点
...
function Content() {
  ...
  const { hour } = useControls({
    hour: { value: 8.8447, min: 0, max: 24, step: 0.01 },
  });
  ...
  useFrame((state, delta) => {
    ...
    // 更新3D Tiles 插件实例的经纬度，然后调用 update() 方法，
    // 将世界坐标系原点定位到指定经纬度附近。
  ...
  useFrame((_, delta) => {
    ...
    const offset = geodetic.longitude/15;
...
export default function CruisingAltitude() {
  return (
    <WebGPUCanvas
      forceWebGL={false}
      shadows
